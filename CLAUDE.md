...
**nightloom-cli** — `chat.rs` (REPL: `/new`, `/quit`; `--resume`/`--continue`; `--tools` runs the agentic loop over the built-ins in `tools.rs`, capped at 8 rounds per user turn — on hitting the cap the last round's results are still recorded so no `tool_use` is left unanswered in the log; Ctrl-C mid-stream cancels the request and records the partial reply with pending tool calls stripped, since a `tool_use` without a result is invalid on replay), `sessions.rs` (session listing / id-prefix lookup), and `probe.rs` (matrix runner, `--target provider:model:thinking-spec[:tools]`).
